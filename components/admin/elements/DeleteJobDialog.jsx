...
export default function DeleteJobDialog({ open, setOpen, slug }) {
	const { mutate } = fetchAllPost()

	const handleDeleteJob = async (slug) => {
		try {
			const response = await fetch(`/api/v1/job/${slug}`, {
				method: 'DELETE'
			})
			if (!response.ok) {
