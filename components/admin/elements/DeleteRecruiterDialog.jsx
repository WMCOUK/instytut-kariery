...
export default function DeleteRecruiterDialog({ open, setOpen, slug }) {
	const { mutate } = fetchAllPost()

	const handleDeleteRecruiter = async (slug) => {
		try {
			const response = await fetch(`/api/v1/recruiter/${slug}`, {
				method: 'DELETE'
			})
			if (!response.ok) {
		...
		<AlertDialog open={open} onOpenChange={setOpen}>
			<AlertDialogContent>
				...
				<AlertDialogFooter>
					...
					<AlertDialogAction
						onClick={() => handleDeleteRecruiter(slug)}
					>
						Confirm
					</AlertDialogAction>
				</AlertDialogFooter>
