...
const ImageUpload = ({ onChange, value, showReuse = true, imgHeight, imgWidth }) => {
	...
	const fetchImages = useCallback(async (cursor = null, isPrev = false) => {
		...
		try {
			...
			setPreviousImages(images.sort((a, b) => new Date(b.created_at) - new Date(a.created_at)))
	...
	const handleImageUpload = useCallback(
		({ event, info }) => {
			if (event === "success") {
				const newImage = { secure_url: info.secure_url, public_id: info.public_id }
		...
		<div className="space-y-4">
			...
			>
				{({ open }) => (
					...
					>
						{value ? (
							<Avatar style={{ height: imgHeight || "150px", width: imgWidth || "100%" }} className="rounded-xl overflow-hidden">
			...
			<div className="flex flex-row gap-2" style={{ width: imgWidth || "100%" }}>
				{value && (
					<Button variant="destructive" size="sm" onClick={handleClearImage} className="flex items-center gap-2">
								...
								<div className="grid grid-cols-2 sm:grid-cols-4 gap-4 max-h-[400px] overflow-y-auto p-2">
									{previousImages.map((image, index) => (
										...
										>
											...
											/>
											<Avatar style={{ height: imgHeight || "140px", width: imgWidth || "auto" }} className="rounded-lg overflow-hidden">
												<AvatarImage src={image.secure_url} alt={`Image ${index + 1}`} className="object-cover h-full w-full" />
