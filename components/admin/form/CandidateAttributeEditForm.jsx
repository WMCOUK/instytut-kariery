...
export default function CandidateAttributeEditForm({ attribute, attPath }) {
	...
	const form = useForm({
		defaultValues: {
			...
			joinDate: attribute?.joinDate || '',
			leaveDate: attribute?.leaveDate || '',
		},
	})
	...
	const handleUpdateAttribute = async (data) => {
		setLoading(true)
		try {
			const response = await fetch(`/api/v1/candidate/${attPath}/${attribute?.slug}`, {
				method: 'PATCH',
			...
			setLoading(false)
			router.push(`/candidate/attributes/${attPath}`)
		}
	}
