...
export default function JobEditForm({ job }) {
	...
	const defaultValues = {
		...
		maxApplicants: job?.maxApplicants || 100,
		skills: job?.skills || [],
		benefit: job?.benefit || [],
		...
		moderation: job?.moderation || "pending",
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
							<div className="space-y-2">
								...
								<Input
									...
									type="number"
									{...register("numberOfPositions", { valueAsNumber: true, onChange: () => trigger("numberOfPositions") })}
									className={cn("h-11", errors.numberOfPositions && "border-red-500")}
								/>
							...
							<div className="space-y-2">
								...
								<Input
									id="maxApplicants"
									type="number"
									{...register("maxApplicants", { valueAsNumber: true, onChange: () => trigger("maxApplicants") })}
									className={cn("h-11", errors.maxApplicants && "border-red-500")}
								/>
							...
							<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
								<div className="space-y-2">
									...
									<Input
										id="minSalary"
										type="number"
										{...register("minSalary", { valueAsNumber: true, onChange: () => trigger("minSalary") })}
										className={cn("h-11", errors.minSalary && "border-red-500")}
									/>
								...
								<div className="space-y-2">
									...
									<Input
										id="maxSalary"
										type="number"
										{...register("maxSalary", { valueAsNumber: true, onChange: () => trigger("maxSalary") })}
										className={cn("h-11", errors.maxSalary && "border-red-500")}
									/>
					...
					<>
						...
						<Card className="p-6">
							...
							<div className="grid grid-cols-2 md:grid-cols-4 gap-6">
								{switchOptions.map(({ id, label }) => (
									<div key={id} className="flex items-center space-x-3">
										<Switch
											id={id}
											checked={watch(id)}
											onCheckedChange={(checked) => setValue(id, checked, { shouldValidate: true })}
				...
				<div className="flex items-center justify-between pt-6">
					...
					<div className="flex items-center space-x-4">
						<Button
							...
							onClick={() => toast.info("Draft saving not implemented.")}
							className="px-6 py-2"
						>
							Save as Draft
						</Button>
						{step === steps.length ? (
							<Button
								...
								className="px-8 py-2 bg-gray-900 hover:bg-gray-800 text-white"
							>
								{isSubmitting || loading ? "Updating..." : "Update Job"}
							</Button>
						) : (
							<Button
								type="button"
								onClick={nextStep}
								disabled={isSubmitting}
								className="flex items-center space-x-2 px-6 py-2 bg-gray-900 hover:bg-gray-800 text-white"
							>
								<span>Next</span>
