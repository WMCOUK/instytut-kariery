...
const columns = (attributePath) => [
	...
	},
	{
		...
		enableHiding: false,
		cell: ({ row }) => <RowActions attribute={row.original} attributePath={attributePath} />,
	},
]

function CandidateAttributeTable({ data = [], totalPage, page, attributePath, isLoading }) {
							...
							<div key={row.id} className="rounded-lg border p-4 space-y-2">
								{row.getVisibleCells().map((cell) => (
									<div key={cell.id} className="flex items-center justify-between py-1">
										<span className="font-medium text-sm">{cell.column.columnDef.header({ column: cell.column })}</span>
										<span className="text-sm">{flexRender(cell.column.columnDef.cell, cell.getContext())}</span>
									</div>
								))}
							</div>
						))}
				...
				<div className="flex flex-col sm:flex-row items-center justify-between space-y-4 sm:space-y-0 space-x-0 sm:space-x-2 py-4">
					...
					<div className="flex space-x-2 w-full sm:w-auto justify-center sm:justify-end">
						<Button
							...
							asChild
							className={`w-24 ${currentPage <= 1 || data.length === 0 ? "opacity-50 cursor-not-allowed" : ""
								}`}
						>
							<Link href={`?page=${Math.max(currentPage - 1, 1)}`} aria-disabled={currentPage <= 1 || data.length === 0}>
								Previous
							</Link>
						</Button>
						...
						<Button
							...
							asChild
							className={`w-24 ${currentPage >= totalPage || data.length === 0 ? "opacity-50 cursor-not-allowed" : ""
								}`}
						>
							<Link href={`?page=${Math.min(currentPage + 1, totalPage)}`} aria-disabled={currentPage >= totalPage || data.length === 0}>
								Next
							</Link>
						</Button>
					</div>
...
function RowActions({ attribute, attributePath }) {
		...
		<>
			...
			<DropdownMenu>
				...
				<DropdownMenuContent align="end">
					<Link href={`/candidate/attributes/${attributePath}/${attribute?.slug}`}>
						<DropdownMenuItem className="flex items-center gap-2">
							<ReceiptText className="h-4 w-4 text-muted-foreground" />
							Details
						</DropdownMenuItem>
					</Link>
					<Link href={`/candidate/attributes/${attributePath}/edit/${attribute?.slug}`}>
						<DropdownMenuItem className="flex items-center gap-2">
...
export default CandidateAttributeTable
