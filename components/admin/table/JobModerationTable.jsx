...
const columns = (openDeleteDialog) => [
	...
	},
	{
		...
		header: ({ column }) => <DataTableColumnHeader column={column} title="Moderation" />,
		cell: ({ row }) => <div className="capitalize">{row.original.moderation}</div>,
	},
	{
		id: "actions",
		enableHiding: false,
...
function JobModerationTable({ jobs = [], totalPage, page, isLoading, moderation, onSearch }) {
	...
	// Delete dialog state lifted here
	...
	const handleSearchChange = (event) => {
		...
		router.push(`/recruiter/job/${moderation}?page=1&search=${encodeURIComponent(value)}`)
	}

	return (
		<div className="w-full">
			{/* Single shared DeleteJobDialog */}
						...
						<TableBody>
							{table.getRowModel().rows.map((row) => (
								<TableRow
									...
									className={`transition-colors ${row.index % 2 === 0 ? "bg-background" : "bg-muted/40"
										} hover:bg-muted`}
								>
			...
			<div className="flex flex-col sm:flex-row items-center justify-between gap-4 py-4">
				...
				<div className="flex gap-2 w-full sm:w-auto justify-center sm:justify-end">
					<Button
						...
						onClick={() =>
							router.push(
								`/recruiter/job/${moderation}?page=${prevBtn}&search=${encodeURIComponent(
									globalFilter
								)}`
							)
						}
					...
					<Button
						...
						onClick={() =>
							router.push(
								`/recruiter/job/${moderation}?page=${nextBtn}&search=${encodeURIComponent(
									globalFilter
								)}`
							)
						}
		...
		<DropdownMenu>
			...
			<DropdownMenuContent align="end">
				...
				<DropdownMenuItem
					...
					onSelect={(e) => {
						e.preventDefault()
						// Open dialog for this job's slug
