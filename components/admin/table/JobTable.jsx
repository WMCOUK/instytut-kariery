...
const columns = (openDeleteDialog) => [
	...
	// 			height={40}
	// 			width={40}
	// 			src={`${row.original.image ? row.original.image : "/images/dummy.jpg"}`}
...
function JobTable({ jobs = [], totalPage, page, isLoading, totalJob, onSearch }) {
	...
	const handleSearchChange = (event) => {
		...
		router.push(`/recruiter/job?page=1&search=${encodeURIComponent(value)}`)
	}
			...
			<div className="flex flex-col sm:flex-row items-center justify-between gap-4 py-4">
				...
				<div className="flex gap-2 w-full sm:w-auto justify-center sm:justify-end">
					<Button
						...
						onClick={() =>
							router.push(
								`/recruiter/job?page=${prevBtn}&search=${encodeURIComponent(
									globalFilter
								)}`
							)
						}
					...
					<Button
						...
						onClick={() =>
							router.push(
								`/recruiter/job?page=${nextBtn}&search=${encodeURIComponent(
									globalFilter
								)}`
							)
						}
