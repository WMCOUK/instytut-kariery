...
import CandidateGrid1 from '@/components/landing/elements/candidates/CandidateGrid1'
import CandidateList1 from '@/components/landing/elements/candidates/CandidateList1'
import CandidateVerticalFilter from '@/components/landing/elements/candidates/CandidateVerticalFilter'
...
import SortDropdown from "@/components/landing/elements/recruiter/SortDropdown"
...
export default function CandidateSection1() {
	...
	const updateQueryParams = useCallback(() => {
		...
		router.push(`/candidates?${query.toString()}`, { scroll: false })
				...
				<div className="grid grid-cols-1 lg:grid-cols-4 gap-5">
					...
					<div className="lg:col-span-3">
						...
						{isLoading ? (
							...
							<div className="mt-5 text-center text-red-500">Error loading candidates: {error.message}</div>
						) : candidates.length > 0 ? (
							<>
								<div className={`grid md:grid-cols-${viewMode === "list" ? "1" : "3"} gap-5 mt-5`}>
									{candidates.map((item, i) =>
										viewMode === "list" ? (
											<CandidateList1 item={item} key={item.id || i} />
										) : (
											<CandidateGrid1 item={item} key={item.id || i} />
										),
									)}
								</div>
								{totalPage > 1 && (
