...
import VerticalFilter2 from '@/components/landing/elements/filter/VerticalFilter2'
...
export default function JobSection2() {
	...
	const { jobs, totalPage, isLoading, error } = fetchQueryJob2(
		...
		filters.jobLocation || '',
		Array.isArray(filters.workMode)
			? filters.workMode
		...
		Array.isArray(filters.createdAtRange)
			...
			: (filters.createdAtRange || '').split(',').filter(Boolean),
		filters.latitude || '',
		filters.longitude || '',
	...
	}, [filters, sortOptions, currentPage, itemsPerPage, updateQueryParams])
