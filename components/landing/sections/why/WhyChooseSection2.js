					...
					<div className="grid grid-cols-1 lg:grid-cols-7 gap-0 lg:gap-7 items-center">
						<div className="col-1 lg:col-span-3 mb-12 lg:mb-0" >
							<div className="wow animate__ animate__fadeInRight animated relative mr-10">
								<img className="jump relative mx-auto rounded-xl w-full z-10 bg-muted" src="/images/about/4.png" alt="" />
						...
						<div className="col-span-4">
							<div className="mb-12">
								<h2 className="text-4xl mt-3 mb-4 font-semibold text-foreground">
									Why choose us?
								</h2>
								<p className="mb-6 leading-loose text-muted-foreground">At our job website, we prioritize delivering an exceptional customer experience that sets us apart from the competition. Here are some reasons why our customers love</p>
							</div>
							<div className="grid grid-cols-2 gap-5">
								{data.map((item, i) => (
									<div className="group" key={i}>
										<div className="bg-card py-7 px-7 rounded-xl group-hover:bg-primary transition duration-200 ease-out hover:ease-in">
											<div className="text-primary rounded group-hover:text-primary-foreground text-3xl mb-5">
												{item.icon}
											</div>
											<h3 className="mb-2 text-xl font-semibold group-hover:text-primary-foreground">{item.title}</h3>
											<div>
												<p className="text-muted-foreground leading-loose group-hover:text-primary-foreground/80"> {item.desc} </p>
											</div>
										</div>
									</div>
								))}
